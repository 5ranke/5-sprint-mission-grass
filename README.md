...
## 스크린샷
<img width="1714" height="1342" alt="sprintmission2" src="https://github.com/user-attachments/assets/9820697c-f923-4d9a-a4b4-94ec31a55d89" />

## 주요 변경사항
- Repository 구현
    - 저장로직 구현
    - FileRepository : 파일 입출력으로 데이터 관리
    - JCFRepository : Map 사용하여 데이터 관리
- FileService 추가
    - 비즈니스 로직 구현(유효성 검사, 검색 기능 등)

- 코드리뷰 반영
    - 도메인 공통
        - createdAt 생성 시 Instant.now().getEpochSecond() 사용
        - update 기능 하나로 합침
        - toString, hashCode, equals 삭제
    - User 도메인
        - email 필드 추가
        - pw -> password 변수명 변경   
    - Channel 도메인
    ...
    - ChannelType 클래스 추가
- 코드 리팩터링
    - 검색 로직 stream filter로 통일
    - 존재확인/중복확인 로직 stream anymatch로 통일
    - 검색 search, 조회 find로 통일
